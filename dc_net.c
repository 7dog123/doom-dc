...
extern struct sockaddr_in dc_net_dnssrv;
...
static int IP_UDPsocket (void)
{
  ...
  // set to non-blocking
  fs_socket_setflags(s, O_NONBLOCK);
...
{
  // enters with DC network initialized by the launcher
  ...
  while (++i < myargc && myargv[i][0] != '-') {
    ...
    if (myargv[i][0] == '.') {
      ...
      uint8 ip[4] = { 127, 0, 0, 1 };
      //if (lwip_gethostbyname (&dc_net_dnssrv, myargv[i], ip) <0)
        I_Error ("gethostbyname: couldn't find %s", myargv[i]);
      IP_sendaddress[doomcom->numnodes].sin_addr.s_addr = *(int *)ip;
    }
    doomcom->numnodes++;
  }
...
void I_InitNetwork (void)
{
  ...
  if ((i = M_CheckParm ("-net")) != 0) {
    ...
    if (p && p < myargc - 1)
      ...
      p = 0; // default to TCP/IP Infrastructure
